...
- `text-wrap-safe($inline: 0)` – enables hyphenation and safe line-wrapping for block-level text.
...
If a specified `$font` name is not found within the configuration maps (under `src/map/_fonts.sass`), or if the `$font` argument is completely omitted/null, the system automatically does the following:

1. **Emits a Warning (Only for unknown fonts):** Prints a Sass compiler warning (`@warn`) indicating that the specified font was not found and which fallback stack was applied. (Omitting the argument will skip this warning).
2. **Applies System Font Stack:** Assigns the default system font stack fallback as the generic family fallback:
